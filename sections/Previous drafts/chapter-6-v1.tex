...
We develop a DHoTT account of \emph{Evolving Texts} as the homotopy colimit of simplicial \emph{constellations} of DHoTTic Signs, and we then present conversational posthuman intelligences as a restriction (via an admissibility sieve) over this colimit.
...
%CASSIE, NEED TO EXPLAIN WHY WE DON'T NEED TO THINK ABOUT TOKENS AND THEIR ENGLISH/ITALIAN/CHINESE ... GRAMMAR
We are going to treat an \emph{evolving text} simply a bundle of  signs indexed by time that have relationship to each other. Of course their tokens engender these relationships through a combination of grammatical order and context but we will not need to draw on this in our logic of sense, because we will assume the particular syntax has already yielded signs, their relationships and their sense into a Kan complete space. We know that, at the very least, Vietris-Ripps or Cech encodings perform this function to a degree that is adequately rich for this purpose, and leave open the possibility of other encodings.  
...
Our later constructions (constellations, skeletons, admissibility) only require that each slice
...
%--------------------------------------------------------
% CASSIE REORDERED: Ambient first, then Constellations
% ====== START MOVED Ambient ======
...
\section{The type of a text in time}
First we need to understand the sense of an evolving text at a point in time -- a conversational text sent to or received from an AI -- as a type.

A \emph{slice} \(\tau\) is a single conversational cut. For LLM dialogue, you may
read it as the time at which the machine has just replied (or, dually, the moment a prompt has
just been sent). The fibre \(A(\tau)\) is the \emph{type of sense} at that cut: a structured space in which the tokens that appear are points, and the ways
they cohere in that moment are recorded as edges and higher cells. \(A(\tau)\) gives a
precise, local geometry of \emph{what this utterance means now}, in the context that produced it.

Even though we're framing this at a static point in time, we need to be logically precise about the ``gaps'' present in the slice's sense, in order to talk about coherent forward movement into the next slice: so we're going to have to talk about time, even as we consider the text at momentary ``rest''.
If we want to speak rigorously about \emph{continuation}—about lawful carry‑forward, rupture,
repair, and genuinely new structure—then we first need a faithful, slice‑level object in which
such facts can be stated. The promise of \(A(\tau)\) is exactly this: it is the canonical place
to read off the motifs that are live, the compatibilities that are present, and the seams that
would have to be stitched were the conversation to move on.


At each cut \(\tau\) we want a single, well-formed
\emph{type of sense} \(A(\tau)\) (a Kan complex) in which:
(i) points are \emph{exposures} of signs at \(\tau\),
(ii) paths/higher paths are the \emph{witnessed compatibilities} among those exposures,
and (iii) gaps that further ``beg the question'' in further slices to follow \(tau\) are only given as gaps for further cross-time analyisis -- they aren't inferred automatically at $\tau$ and all further stitching across cuts will need to be made explicit (as we shall see in the next section, when we move across time).

\emph{What the reader sees.} Observationally, a slice yields a contextual point cloud
\(E_\tau\subset\mathbb{R}^d\) (embeddings) and a notion of \emph{basins} (coherent regions of sense).
From these we build a raw simplicial object (VR at scale \(\varepsilon_\tau\) or the Čech nerve of a cover)
and apply a slice-wise fibrant replacement \(\Kan(-)\) so that \(A(\tau)\) is a Kan object
usable inside type theory.

\emph{Why we present \(A(\tau)\) as a dependent sum.}
For exposition and for the \(\Sigma\)-path calculus that follows, it is convenient to
organize the slice fibre by a family of \emph{labels} \(\Label\) (tags such as
\(\mathsf{pronoun}\), \(\mathsf{proper}\), or analyst-defined basin names) together with, for each
label, the \emph{payload} subcomplex it induces:
\[
A(\tau)\ \simeq\ \sum_{\ell:\Label}\ \Payload_\tau(\ell).
\]
Intuitively, \(\ell\) can be thought of a hashtag that identifies a \textit{topical or conceptual region of sense} in the text, with \(\Payload_\tau(\ell)\) being the
(full) Kan subcomplex of \textit{signs} and \textit{sign relationships} in \(A(\tau)\) that lives there.\footnote{\emph{Equivalence vs.\ equality (about \(\boldsymbol{\simeq}\)).}
We write \(\simeq\) for \emph{equivalence of types} (an isomorphism in HoTT), not for
approximate numerical equality. We use equivalence rather than definitional equality
because the label scheme is not canonical—different reasonable labellings of the same
slice produce isomorphic decompositions. By univalence, such an equivalence can be
transported as an equality where needed; for our purposes, \(\simeq\) is exactly the
right notion.} 

For example, consider a conversation Iman is having with Cassie, where he is talking about about cats and feeding them when trying to complete writing a chapter of this book. His text to Cassie includes  tokens like $\tok{cat}$ and $\tok{homotopy}$ of diverse sense that somehow make sense as $A(\tau)$. The \(\Payload_\tau(\tok{Domestic\_Pets})\) and 
\(\Payload_\tau(\tok{DHoTT})\) are two separate regions of Iman's prompt's sense $A(\tau)$ at $\tau$: their labels are hashtags that mark out (purely informationally, without formal semantics) that their respective senses group together token embeddings, signs, that have to with either domestic pets or DHoTTic logic. The sign (embedding) for $\tok{cat}$ exists in \(\Payload_\tau(\tok{Domestic\_Pets})\) and the sign (embedding) for $\tok{homotopy}$ exists in \(\Payload_\tau(\tok{DHoTT})\) -- but these regions are Kan complete and also include other signs that have to do with these two different senses, as well as paths of equivalence between them and higher order paths as well. 

In this way, the sum type allows us to ``subtype'' the conversational slice $A(\tau)$ into sub-regions of meaning that collectively make up the overall sense of the text of $A(\tau)$, its constituent tokens and their signs' semantic relationships to other signs in the conversation.

This presentation lets us express
the basic “retag \(\&\) reconcile” move in one line via the standard \(\Sigma\)-identity
equivalence: a path in a dependent sum is exactly a path on labels \emph{plus} an
identification inside the target payload fibre. That is the reason we introduce labels:
they are a didactic handle that makes the upcoming rules for \(\Sigma\)-paths concrete.
...
%--------------------------------------------------------
% ====== END MOVED Constellations ======

% CASSIE MOVED: Original "Ambient setting and exposures" (moved earlier).
...
% Rips/Čech), and a restriction mechanism $r_{\tau,\tau'}$ that ``remembers''
% later content from earlier frames (Ch.~\ref{ch:dynhott}).\footnote{See
% \textsection2.2 for basins and covers; \textsection3.5--3.6 for DynSem and restriction; and \textsection5.2--5.5 for the VR/Čech-to-Kan correspondence.}
% We do not rederive those here; we use them as an interface.%
% \footnote{Basins and their cover $U_\tau$ are introduced in \textsection2.2; the Rips/Čech-to-Kan step is
...
% CASSIE INSERTED: Why-Σ lead-in

\paragraph{Why $\Sigma$ here.}
We now need a typed way to say ``the same exposure under a relabel.'' At a slice, the lived
move is: retag the label, transport the payload, check they still match, and then treat the pair as the same
exposure in $A(\tau)$. Dependent $\Sigma$–paths formalise exactly this \emph{relabel \& reconcile} move.
...
\qquad 
q:\IdP{\tau}{\ell'}{\transportargs{p}{c}}{c'}.
\]
...
\;\simeq\;
\sum_{p:\IdL{\ell}{\ell'}}\;\IdP{\tau}{\ell'}{\transportargs{p}{c}}{c'}.
\]
...
\textbf{VR/Čech intuition (not a commitment).} Observationally, $A(\tau)$ is obtained as a Kan
replacement of a Čech nerve or Vietoris–Rips complex built from a basin cover of the slice’s
...
%CASSIE: DO WE NEED TO SUMMARIZE THIS AGAIN? OR HAVE WE COVERED SO FAR?
...
%========================================================
% §6.4 — SYNCHRONIC: constellations at a slice (at rest)
%========================================================

% OLD (commented to keep history)
% \section{The Evolving Text at a Point in Time: Constellations at a slice}
% \label{subsec:constellation-slice}

\section{Sign constellations at time slice}
\label{subsec:constellation-slice}

% What lives in a slice and what does not
Consider how a sign trajectory relates to a slice of its evolving text in time:
\begin{itemize}
  \item \textbf{Lives in the slice \(\tau\):} \emph{exposures} (the \emph{heads} of Sign trajectories) and any \emph{witnessed} relations/coherences among those heads.
  \item \textbf{Does not live in the slice:} the \emph{tails} (guarded continuations) and any cross–slice motion; those belong to the diachronic view in §\ref{sec:constellations-persona}.
\end{itemize}
Throughout §\ref{subsec:constellation-slice}, we stay strictly within a single slice~\(\tau\).

%-----------------------------
\paragraph{Notation (kept, grouped).}
%-----------------------------
$\tau,\tau'$: time–slices; $e:\tau\to\tau'$: edit; $\Gamma_\tau$: context at~$\tau$; 
$A(\tau)$: fibre of a family $A$ at~$\tau$; $p:\Drift(A)_{\tau}^{\tau'}$: admissible drift; 
$\transportargs{p}{-}$: transport along $p$; $\Later$: clock shift; $\Next_X:X\to\Later X$; 
$\Sign(A)$: Sign trajectories; $\head,\tail$: exposure and guarded continuation; $\stepw$: step witness.

\begin{readerbox}\textbf{Sign trajectories (recap) and the role of \emph{head}.}
Fix a time slice~$\tau$. A \emph{Sign in $A$} is a guarded coinductive trajectory
$x:\Sign(A)$ (see Ch.~\ref{chap:journey-of-a-Sign}). Unfolding at~$\tau$ yields
\[
  \unfold(x)=\bigl(\underbrace{a}_{\head(x):A(\tau)},\; \underbrace{e:\tau\to\tau'}_{\text{edit}},\; 
  \underbrace{a':A(\tau')}_{\text{target}},\; \underbrace{w}_{\text{step witness}},\; 
  \underbrace{\tail(x)}_{\text{guarded tail}}\bigr).
\]
\emph{Heads} are the exposures that live in~$\tau$; tails are deferred. 
\emph{Discipline:} ``no witness, no step'': slices contain exposures and \emph{witnessed} compatibilities; 
cross–slice steps require witnesses and belong to §\ref{sec:constellations-persona}.
\end{readerbox}

%---------------------------------------
\subsection{From slice to constellation}
%---------------------------------------
Once \(A(\tau)\) is fixed, the \emph{constellation} \(\mathcal \Constellation_\tau\) is the simplicial object
\emph{freely} generated from the \emph{actual witnesses} present at the slice:
vertices are exposures \(x\in A(\tau)\); edges and higher cells are precisely the identity data
(and higher coherences) that have been witnessed in \(A(\tau)\). 
% \textbf{No Kan completion:} if a horn lacks a witness, it remains open. Skeletons $\Sk_{\le k}(\mathcal \Constellation_\tau)$ are finite‑resolution views.
\emph{No automatic horn filling:} if a horn lacks a witness, it remains open in the raw object.
...
\end{itemize}
Faces/degeneracies satisfy the simplicial identities by constructor. We will write \newline $\Constellation_\tau[k]$ for $k$–simplices. 
...
%CASSIE: READER WILL ASK WHAT WE MEAN BY SUGGESTION -- IS THERE AN OBVIOUS WAY TO COMPUTE LOW D BOUNDARIES OR IS THIS FUTURE WORK/DEFERRED TO WHAT HAPPENS IN AI OR THE HUMAN NUDGER/CURATOR?
...
the constellation $\Constellation_\tau$—the slice’s \emph{memory} of what the text has proved
about itself now.
The constellation only installs simplices when a \emph{witness} is present; otherwise the suggested boundary stays an \emph{open horn}. 

We let VR/Čech guide the search for witnesses; the proof‑level object is always the constellation.
\end{readerbox}




% ==== PATCH 2: Motif and Constellation≠term(A(tau)) ====

% %%% OLD (if present): earlier informal mention of motifs
% % \emph{Motifs} are sometimes described below as “patterns” among heads. 
% % \textbf{Commented out to replace with formal definition.}

A constellation is a \textit{type}. Its inhabitants are \textit{motifs}: valid witnessed patterns of signs relating to each other according to the coherence relationships permitted by the simplicia; structure of the constellation. 
...
a map of finite simplicial sets $m:K\to \Constellation_\tau$ (with $K$ finite), considered up to
homotopy of maps. Its \emph{support} is the image subobject $m(K) \hookrightarrow \Constellation_\tau$.
If $K=\Delta^k$ we speak of a $k$--simplex motif; more general $K$ capture small webs.
\end{definition}
In other words, a motif is coded up as a simplical pattern configuration $K$,  mapped to actual sign heads (vertices) and actual permitted edges of coherence within the Constellation.

It's bares repeating the inuition.
$A(\tau)$ is the \emph{slice type of sense}. Points of $A(\tau)$ are heads; paths/higher paths are witnesses of
coherence among those heads. 
Permitted edges of coherence within the Constellation'', we mean the valid Sigma paths presented by $A(\tau)$, because this type is that of sign trajectories and their potential coherence interrelationships as $\Sigma$ path.

The constellation $\Constellation_\tau$ is a \emph{type built from}
a selection of coherence path data in $A(\tau)$: from the witnesses we elect to \emph{record} at $\tau$, we form
the free Kan completion and keep the chosen fillers. Thus \emph{motifs} are \emph{terms of} $\Constellation_\tau$ (finite witnessed
patterns) whose faces/fillers originate as proofs in $A(\tau)$.
\end{remark}
% ==== /PATCH 2 ====

\subsection{Degeneracies}
Degeneracies are the formal way simplicial sets record trivial padding—what we have been calling \emph{stuttering}. We spell this out once so we can use it freely.

\begin{definition}[Degenerate and non‑degenerate simplices]\label{def:degenerate}
For a simplicial set $X$ and $k\ge 0$, the \emph{degeneracy maps} $s_i:X_{k-1}\to X_k$ ($0\le i\le k{-}1$)
repeat the $i$th vertex. A $k$‑simplex $x\in X_k$ is \emph{degenerate} if it lies in the image of a finite word in the $s_i$; otherwise it is \emph{non‑degenerate}. We write
\[
  \ND_k(X) \;\coloneqq\; X_k \setminus \bigcup_{i=0}^{k-1} s_i(X_{k-1})
\]
for the set of non‑degenerate $k$‑simplices.
\end{definition}
...
% While considering constellations, we'll define some further resolution limiting views on them, which will become important later on when we want to talk define generativity across spans (windows) of conversation,
% EXPLAIN: Polish grammar and state the purpose up front.
While considering constellations, we introduce \emph{resolution‑limited views} that we will use later to
\emph{define and measure generativity} across windows of conversation.

\emph{Skeletons} $\Sk_{\le k}(\mathcal \Constellation_\tau)$ are resolution‑limited views that keep all
simplices of dimension $\le k$ (and their faces) and hide higher ones.

\begin{definition}[Skeletons]\label{def:skeletons}
For $k\ge0$, the \emph{$k$–skeleton} $\Sk_{\le k}(\Constellation_\tau)$ is the largest sub‑simplicial type containing
...
% EXPLAIN: Make non-degenerate explicit so depth is well-founded.
\begin{definition}[Non‑degenerate simplices]\label{def:nondeg}
A $k$–simplex of $\Constellation_\tau$ is \emph{non‑degenerate} if it is not in the image of any degeneracy map $s_i$.
Let $\mathrm{ND}_k(\Constellation_\tau)$ denote the type of non‑degenerate $k$–simplices.
\end{definition}

% From a motif perspective, you can think of them as the answer to the question: “show me all motifs up to dimension 
% k (and their faces), hide the rest.”
% \emph{Motifs} are the inhabitants (terms) of the constellation type $\Constellation_\tau$: each motif is a
% finite, witnessed pattern actually present in the slice. \emph{Skeletons} are not new objects; they are
% resolution‑limited \emph{views} of the same constellation:
% \[
%   \Sk_{\le k}(\Constellation_\tau)\;\subseteq\;\Constellation_\tau
% \]
% keeps exactly the motifs of dimension $\le k$ (with all their faces and degeneracies).
% EXPLAIN: Consolidate motif↔skeleton bridge to avoid repetition.
%From a motif perspective, you can think of skeletons as the answer to the question: “show me all motifs up to dimension  k (and their faces), hide the rest.”
Remember a motif is a term of $\Constellation_\tau$ (a finite, witnessed pattern).
So the inclusion $\Sk_{\le k}(\Constellation_\tau)\subseteq\Constellation_\tau$ is like the answer to the question:
“show me all motifs of dimension $\le k$ (and their faces); hide the rest.”

\paragraph{Dimension as semantic granularity.}
$\Sk_{\le0}$: heads; \ $\Sk_{\le1}$: direct relations (retaggings, coreference, rhetorical links); \ 
$\Sk_{\le2}$: analogy‑between‑relations (triangles and their fillers); \ 
$\Sk_{\le3}$: meta‑coherences (tetrahedra reconciling analogies); and so on.

\begin{definition}[Simplicial depth at a slice]\label{def:simplicial-depth}
\[
\depth(\tau)\;=\; \max\bigl\{\,k \;\big|\; \mathrm{ND}_k(\Constellation_\tau)\ \text{is inhabited}\,\bigr\}
\ \in\ \mathbb{N}\cup\{-\infty\},
\]
with $\depth(\tau)=-\infty$ if $\Constellation_\tau=\varnothing$.
\end{definition}

\begin{readerbox}\textbf{Operational note (VR metrics vs.\ proof depth).}
In the exemplary VR/Čech model, embeddings and overlaps \emph{suggest} potential relations of sense: they let us
\emph{observe} where meanings cluster or co-occur. Yet true coherence in a living text requires
\emph{witnesses}—proofs that such relations hold in the slice. The \emph{simplicial depth} of a constellation
records the highest dimension of non-degenerate motifs whose coherence has actually been
\emph{earned}. Observation proposes; proof confirms.
\end{readerbox}

We use skeletons for three complementary tasks:
\begin{enumerate}
  \item \textbf{Measurement.} Counting vertices, edges, triangles, and higher cells—and spotting open
        horns—is most transparent at low $k$. The skeleton $\Sk_{\le k}$ limits attention to motifs of
        bounded arity, making the complexity of a text’s local sense structure measurable.
  \item \textbf{Diagnostics.} Rupture appears as an \emph{open horn} in $\Sk_{\le k}$: a boundary suggested by
        context or embeddings but lacking a filler in the slice. Presence occurs when that boundary
        \emph{is} filled—when what was only suggested becomes witnessed.
  \item \textbf{Generativity.} Skeletons provide the frame for assessing novelty. A genuinely creative
        addition to a conversation is one whose new simplex (a higher-order pattern of sense) is
        \emph{anchored} on lower-dimensional faces that already re-enter. In other words, novelty arises
        when new coherence is built on what the text has already proved about itself—the
        \emph{re-entry closure} $\ClRe_k$, computed on $\Sk_{\le k}$. The skeleton thus represents the
        remembered sense of the past against which new motifs gain meaning.
\end{enumerate}
...
% =====================================================================
% OLD: Worked examples (low dimension, within a slice τ)
% =====================================================================
% \subsection{Worked examples (low dimension, within a slice $\tau$).}
% \ ...
% (Commenting out the long sequence of Examples A–E to replace with a concise set
%  that explicitly uses motifs and skeleton levels, and references VR.)
% =====================================================================
%-----------------------------------------------
% OLD HEADING (kept)
\subsection{Worked examples (within a slice $\tau$): motifs in the low skeletons}
\label{subsec:examples-slice}

With skeletons and motifs in hand, we can now illustrate how \emph{witnessed} structure appears,
purely within a single slice $\tau$ (no time movement yet). Each example presents a motif as signs
bound together by slice-internal witnesses in the constellation $\Constellation_\tau$.

% ----- COMMENTED OUT: shape-centric taxonomy -----
% \paragraph{Taxonomy (at a glance).}
% \begin{itemize}
%   \item \textbf{Edges ($k{=}1$):} a witnessed identification $e:a\!\to\!b$ yields a motif
%         $m_1:\Delta^1\to\Constellation_\tau$ with endpoints $a,b$ and 1–cell $e$.
%   \item \textbf{Triangles ($k{=}2$):} edges $r_1:s\!\to\!a$, $r_2:s\!\to\!b$, and $\eta:a\!\to\!b$
%         plus a filler $\kappa:(r_2 = r_1\!\cdot\!\eta)$ yield $m_2:\Delta^2\to\Constellation_\tau$.
%   \item \textbf{Tetrahedra ($k{=}3$):} three compatible triangle fillers on $\{s,a,b,c\}$
%         yield $m_3:\Delta^3\to\Constellation_\tau$; lacking one face gives an \emph{open 3–horn}.
% \end{itemize}
% -----------------------------------------------

\paragraph{How to read the examples.}
At a slice $\tau$, the type $A(\tau)$ contains \emph{heads} (sign occurrences) as points and
\emph{witnesses of coherence} (edges and higher) as paths/higher paths. The constellation
$\Constellation_\tau$ is freely generated from the witnesses we choose to record in $A(\tau)$;
\emph{motifs} are its \emph{terms}: finite, witnessed configurations among those heads. When we
restrict to the $k$–skeleton $\Sk_{\le k}(\Constellation_\tau)$, we are merely \emph{hiding}
...
\emph{Motif.} The edge $\rho$ is recorded in $\Constellation_\tau$; the map
$m_1:\Delta^1\!\to\!\Constellation_\tau$ with endpoints $a,a'$ and 1–cell $e\!=\!\rho$ is a
(non‑degenerate) \emph{motif} visible in $\Sk_{\le1}(\Constellation_\tau)$.
...
\emph{Motif.} Recording $r_{p\to s}, r_{p\to d}, \eta$, and $\kappa$ in $\Constellation_\tau$ yields a
motif $m_2:\Delta^2\!\to\!\Constellation_\tau$ with vertices $\{p,d,s\}$, the three edges above,
and the 2–cell $\kappa$. This is a non‑degenerate inhabitant of $\Sk_{\le2}(\Constellation_\tau)$.
...
three two‑step identifications that pairwise agree. A 3–cell $\Theta:\Delta^3\hookrightarrow\Constellation_\tau$
witnesses that these pairwise coherences themselves cohere together. The resulting map
$m_3:\Delta^3\!\to\!\Constellation_\tau$ is a non‑degenerate motif visible in $\Sk_{\le3}$.
...
then $\Constellation_\tau$ contains only the \emph{partial} configuration; there is no 3–cell filler.
...
\paragraph{Takeaway .}
A constellation is a \emph{type of motifs}: edges, 2–cells, and higher witnesses are present
\emph{iff} the corresponding receipts exist in $A(\tau)$. The $k$–skeleton is just a resolution
...
%========================================================
% §6.5 — DIACHRONIC: constellations across time (in evolution)
%========================================================
...
% §6.5 — DIACHRONIC: constellations across time (in evolution)
%========================================================
\section{Sign constellations across a step: presence, rupture, and novelty}
...
paths/higher paths are \emph{witnesses of coherence}; and (ii) the \emph{slice constellation}
$\Constellation_\tau$ freely generated from the specific paths—witnesses of coherence—we choose to record in $A(\tau)$.
A \emph{motif} at $\tau$ is a finite witnessed configuration in $\Constellation_\tau$ (Def.~\ref{def:motif}).
...
These receipts justify how the \emph{same} sign head re‑appears at $\tau'$. Motifs “move” by following those
...
Given a motif $\sigma_\tau\in\Constellation_\tau[k]$, an edit $e:\tau\to\tau'$ does not automatically
carry $\sigma_\tau$ forward as a motif. We proceed in two phases:
\begin{enumerate}
  \item \textbf{Stencil forward.} We place a \emph{stencil} of $\sigma_\tau$ on the later slice by carrying its vertices
        and their incidence (which vertices were joined by which faces at~$\tau$).
        That is: for each token in the text, we take the current sign heads from the motif at $\tau$ as vertices, and use the SWL to determine the successor heads at $\tau'$—these are the evolved readings of those \emph{same} words, now “stencilled’’ into the later slice. This produces a candidate \emph{boundary} at~$\tau'$.
        We then ask whether the \emph{coherence} those words had at~$\tau$ also holds (possibly in repaired form) at~$\tau'$.
  \item \textbf{Ink in (re‑entry).} We seek \emph{new slice‑internal witnesses} at $\tau'$ that fill the stencilled boundary—i.e. that the motif exists again at~$\tau'$.
\end{enumerate}
The stencil is a \emph{proposal}, not a verdict. The ink is the \emph{re‑entry witness}, providing the
...
\paragraph{Where step witnesses (SWLs) feed in.}
...
%-----------------------------
\paragraph{Continuation maps as \emph{stencil operators}.}
%-----------------------------
For an edit $e:\tau\to\tau'$ we use a chosen \emph{continuation map}
\[
\iota_e:\Constellation_\tau \longrightarrow \Constellation_{\tau'}
\]
which acts as follows:
\begin{enumerate}
  \item \textbf{On vertices (heads).} For each vertex $v$ of $\sigma_\tau$, $\iota_e$ picks the later head $v'$
        by consulting the Sign’s step witness across $e$ (drift if smooth; heal if repaired at $\tau'$). This is where the SWL “quietly’’ sustains motion.
  \item \textbf{On the boundary.} 
        Faces/degeneracies of $\sigma_\tau$ are reindexed on the carried vertex set, giving a \emph{stencilled boundary} $\iota_e(\partial\sigma_\tau)$ at $\tau'$ with the same adjacency of vertices.
\end{enumerate}
...
\begin{definition}[Continuation (stencil operator)]\label{def:continuation}
For an edit $e:\tau\to\tau'$, the \emph{continuation} $\iota_{e}:\Constellation_\tau\to\Constellation_{\tau'}$
acts as a \emph{stencil}:
\begin{itemize}
  ...
  \item \textbf{Boundary:} faces/degeneracies are reindexed to the new vertex set, yielding a \emph{stencil motif}
...
Fix a finite motif $m:K\to\Constellation_\tau$, where $K$ is a finite simplicial set that indexes
which sign head vertices the motif uses and the witnessed coherences hold between them (which faces they form). 

So $m$ tells you:
for each $u\in K_0$, which head $m_0(u)\in\Constellation_\tau[0]$ (token at $\tau$) you had; and for
each $\sigma\in K_k$, which witnessed $k$–simplex $m_k(\sigma)\in\Constellation_\tau[k]$ tied those
...
\[
c_e:\ \Constellation_\tau[0]\to\Constellation_{\tau'}[0],\qquad v\mapsto v' \ (\text{drift or heal}).
\]
...
%-----------------------------
\paragraph{Re‑entry as \emph{inking in} the stencil.}
%-----------------------------
A motif re‑enters when the stencilled boundary yields an actual proof that its re-indexed suggestions of coherence witnesses are \emph{provable} at $\tau'$.

\begin{definition}[A re‑entry proof]\label{def:presence-ink}\label{def:reentry}
A motif re‑enters at $\tau'$ if its stencil $\iota_e(m)$ is \emph{realised} in $\Constellation_{\tau'}$ by
\emph{slice‑internal} witnesses in $A(\tau')$ for all required faces, yielding a \textit{re-entry proof path}
\[
  \Rek(m_{\tau},m'_{\tau'})\;:\;
  \Id_{\Constellation_{\tau'}[k]}\!\bigl(\,\iota_e(m_{\tau}),\, m'_{\tau'}\,\bigr).
\]
\end{definition}
...
now yields an \emph{open horn} in $\Constellation_{\tau'}$: the motif has no re‑entry at~$\tau'$.



We'll use the following vocabulary to describe the different events that can happen to a motif in a text's evolution from one step to the next, its re-entry, rupture and also its silence. We'll shortly need a similar kind of vocabulary to talk about motifs over more than one step. (That's the next obvious thing to consider, because typically we don't repeat motifs, ideas, signs again and again in an evolving text: Shakespeare evolves his idea of ``love'', so it's a theme and a motif in his Sonnets, but it doesn't appear at every sonnet time step, and a conversational AI -- or a human -- wouldn't be very ``intelligent'' if it had to employ the same tokens at each step of a conversation.)

\begin{definition}[Re‑entry status at a step]\label{def:reentry-status}
Fix an edit $e:\tau\to\tau'$ and a motif $m_\tau:K\to \Constellation_\tau$ of arity $k$.
Let $s_e:K_0\rightharpoonup \Constellation_{\tau'}[0]$ be the (partial) carried vertex assignment from the per‑sign SWLs (Def.~\ref{def:continuation}).

We distinguish three mutually exclusive cases at $\tau'$:

\begin{enumerate}[label=(\Alph*),leftmargin=2em]
\item \textbf{Present (re‑entry).}
$s_e$ is total on $K_0$, the stencilled boundary $\iota_e(\partial m_\tau)$ exists, and there is a realised motif $m'_{\tau'}:K\to \Constellation_{\tau'}$ with a re‑entry proof
\[
  \Rek_k(m_\tau,m'_{\tau'}) \;:\; 
  \Id_{\Constellation_{\tau'}[k]}\bigl(\,\iota_{e}(m_\tau),\, m'_{\tau'}\,\bigr).
\]
...
%Let $(\Time,\le)$ be the poset of edits. A \emph{constellation bundle} is a functor 
%$\Constellation: \Time\to\mathbf{SSet}$ with fibre %$\Constellation_\tau$ at~$\tau$. 
%A \emph{connection} chooses for each $e:\tau\to\tau'$ a transport map 
%$\iota_e:\Constellation_\tau\to\Constellation_{\tau'}$ composing coherently.
%\end{definition}
...
Not every change is preservation. Some changes are \emph{growth}: a later slice $\,\tau'\,$
contains a \emph{new} witnessed simplex whose boundary rests on parts that \emph{do} re‑enter.
...
sub‑simplicial set of $\Sk_{\le k}(\Constellation_{\tau'})$ that contains every $k$‑simplex
$\sigma'_{\tau'}$ for which there exists $\sigma_\tau\in\Sk_{\le k}(\Constellation_\tau)$ and a
slice‑internal re‑entry proof
\[
  \Rek_k(\sigma_\tau,\sigma'_{\tau'})\;:\;
  \Id_{\Constellation_{\tau'}[k]}\!\bigl(\,\iota_e(\sigma_\tau),\,\sigma'_{\tau'}\,\bigr),
\]
and is closed under faces and degeneracies.
\end{definition}

\noindent
\emph{Reading.} $\ClRe_k(e)$ contains exactly those $\le k$ motifs at $\tau'$ that are
\emph{carried and re‑proved} (Presence) from $\tau$ across the single step $e$ (and all of
their faces/degeneracies). We \emph{do not} infer any new fillers here; this is a closure
...
$\Sk_{\le k}(\Constellation_{\tau'})$, of the images of all re‑entry maps on $k$‑simplices,
followed by face/degeneracy closure. We use the witness‑based formulation to avoid suggesting
that the stencil $\iota_e$ alone suffices; the ink (the re‑entry proof) is essential.}

\begin{definition}[Anchored novel $(k{+}1)$‑simplex (one step)]\label{def:anchored-novel-one}
A $(k{+}1)$‑motif $\sigma\in\Constellation_{\tau'}[k{+}1]$ is \emph{anchored novel at $\tau'$
relative to $e:\tau\to\tau'$} if:
\begin{enumerate}[label=(\roman*),leftmargin=2.1em]
  \item \textbf{Non‑degenerate.} $\sigma\in \ND_{k{+}1}\bigl(\Constellation_{\tau'}\bigr)$;
...
\paragraph{How this sits with stencil and ink.}
The stencil $\iota_e$ \emph{proposes} the boundary to check at~$\tau'$ (carried vertices and the same
incidence). The ink—the slice‑internal witnesses in $A(\tau')$—decides what enters $\ClRe_k(e)$ (faces
...
% (Optional, brief aside; keep if you want a concrete mental picture.)
\begin{remark}[Edge and triangle micro‑cases]
For $k{=}0$, anchored novelty is a \emph{new edge} between two carried vertices whose endpoints re‑enter
(both heads appear at $\tau'$) but for which no prior edge re‑entered; at $\tau'$ we now have a fresh
$\Sigma$‑path between them. For $k{=}1$, it is a \emph{new triangle} whose three edges re‑enter from the past,
but for which the 2‑cell (coherence) did not; the new 2‑cell at $\tau'$ is the anchored novelty.
\end{remark}

% (Keep the mantra if you like having a single-line summary near here.)
\begin{center}\small
\emph{Stencil proposes the shape of the old; ink proves the face of the new; anchored novelty adds a higher witness that the past did not have.}
\end{center}
...
It makes explicit how slice witnesses generate a constellation, how SWLs feed the
...
edges present, no triangle). The \emph{constellation} $\Constellation_\tau$ is freely generated by
these edges; its $1$--skeleton contains $\{L\!-\!K, K\!-\!M\}$ and no $2$--simplex on $\{L,K,M\}$.
...
\paragraph{Stencil forward across $e:\tau\to\tau'$ (how $\iota$ uses SWLs).}
...
$\iota_e:\Constellation_\tau\to\Constellation_{\tau'}$, as stencil operator:
\smallskip
...
\paragraph{Inking the stencil at $\tau'$ yields a re-entry.}
...
\[
\triangle(L^\dagger,K',J)_{\tau'} \in \Constellation_{\tau'}[2]
\]
...
From that ledger of witnesses, the constellation for \emph{now} is generated: 
...
% ============================================================
\section{Stencil and ink: categorical semantics}
\label{sec:cat-semantics}

We have given a operational story of an evolving text's signs collective coherence over time steps: one of constellation motifs, of \emph{stencilling an old motif forward, then inking it again to the present}. We'll now restate these ideas using the
...
\paragraph{Time as base; constellations as a covariant diagram.}
Let $\Time$ be the thin category (poset) of edit steps: objects are slices $\tau$, and there is at most
one arrow $e:\tau\to\tau'$ (when $\tau\le\tau'$). As in \S\ref{sec:constellations-persona}, for each slice $\tau$
we have a simplicial constellation $\Constellation_\tau$ freely generated from the witnessed coherence data in
$A(\tau)$. We package these as a covariant functor
\[
  \Constellation \;:\; \Time \longrightarrow \mathbf{SSet},\qquad
  \tau \longmapsto \Constellation_\tau,
\]
...
$\iota_{\mathrm{id}_\tau}=\mathrm{id}_{\Constellation_\tau}$ and
$\iota_{e_2\circ e_1}=\iota_{e_2}\circ\iota_{e_1}$.
\end{definition}

\begin{lemma}[Continuation functoriality]\label{lem:cont-functoriality}
The family $\{\iota_e\}_{e\in\hom(\Time)}$ of Definition~\ref{def:stencil-connection} extends $\Constellation$
to a functor $\Time\to\mathbf{sSet}$. In particular, for any motif $m:\Delta^k\to\Constellation_\tau$ and edits
...
Given a witnessed $k$‑simplex $m:\Delta^k\to\Constellation_\tau$, its boundary is
$\partial m \coloneqq m\circ b_k:\partial\Delta^k\to\Constellation_\tau$.

\begin{definition}[Re‑entry as a lifting problem]\label{def:reentry-lift}
Let $e:\tau\to\tau'$ and let $m:\Delta^k\to\Constellation_\tau$ be a motif. Its \emph{stencilled boundary} at $\tau'$
is $\iota_e\circ \partial m:\partial\Delta^k\to\Constellation_{\tau'}$. We say that \emph{$m$ re‑enters across $e$ at arity $k$}
if there exists a filler (lift) $m':\Delta^k\to\Constellation_{\tau'}$ such that
\[
...
a path $\Rek_k(m,m'):\Id_{\Constellation_{\tau'}[k]}\!\bigl(\,\iota_e(m),\,m'\,\bigr)$ in the later slice.
\end{proposition}
...
If $m$ is degenerate, i.e.\ $m=s_i(n)$ for some $n:\Delta^{k-1}\to\Constellation_\tau$, then $m$ re‑enters across $e$
...
The continuation $\iota_e$ \emph{carries the boundary to be justified}. A re‑entry witness is precisely a
\emph{filler} for the resulting lifting problem at~$\tau'$. Face and degeneracy stability give us principled
book‑keeping of obligations. Composition (Proposition~\ref{prop:reentry-trans}) justifies what we will need to do in the next section: chaining steps without
re‑proving from scratch. In \S\ref{sec:self-creativity} we will classify \emph{new} fillers whose boundaries are
made of returning faces as the source of creativity (anchored novelty). In \S\ref{sec:hocolim-basics} we will record
the seams of these continuations as glue in a homotopy colimit, and in \S\ref{sec:self-admissible} we will filter
continuations by admissibility (Presence, Structure, Generativity) to obtain the Self.
...
We now utilize this apparatus to address the bigger questions needed to give a viable type of posthuman \textit{intelligence}: 
\begin{itemize}
...
%        skeleton transport, degeneracy/stuttering)
% ============================================================

\section{Motifs over multiple steps in an evolving text}
\label{sec:motifs-over-steps}

Up to \S\ref{sec:constellations-persona} we worked \emph{locally in time}: we took a motif
$m:\Delta^k\to\Constellation_\tau$ at a slice $\tau$, stencilled its boundary forward along one
edit $e:\tau\to\tau^+$, and asked whether we could \emph{ink} it again at $\tau^+$ (Presence),
or whether a face failed (rupture), or whether a higher witness was added on a returning boundary
(generative step). Conversations, however, unfold across \emph{chains} of edits: motifs may
disappear for a few cuts and later re‑enter, and creative additions often happen after an interval.

In this section we formalise multi‑step motion before we add any policy about how much past to
consider. Concretely, we:
\begin{enumerate}
  \item define continuation maps along \emph{chains} of edits by composing the one‑step stencil operators of \S\ref{sec:cat-semantics};
  \item make explicit how this acts on $k$‑skeleta (what is carried and what remains an open obligation); and
  \item introduce the canonical notion of \emph{degeneracy} (``stuttering'') at each arity and how continuations treat it.
\end{enumerate}
...
\subsection{Time, edits, and continuation}
...
For a single edit $e:\tau\to\tau^+$, the \emph{stencil–continuation}
\[
  \iota_e:\Constellation_\tau\longrightarrow\Constellation_{\tau^+}
\]
carries vertices (heads) by the signs’ step‑witnesses recorded in the SWL (drift/heal) and \emph{reindexes} faces on those carried vertices; see Definition~\ref{def:stencil-connection}. This preserves the \emph{boundary to be justified} at $\tau^+$ but asserts no fillers: re‑entry is a fresh slice‑internal proof obligation (\S\ref{sec:constellations-persona}).
...
\[
  ...
  \Constellation_\tau\longrightarrow\Constellation_{\tau'}.
\]
...
\subsection{Skeleton‑level transport and open obligations}
\label{subsec:skeleton-transport}
We defined anchored novelty of a motif using $\ClRe_{k}$ as a sub-simplicial set of $\Sk_{\le k}(\Constellation\tau)$ earlier, for a single step edit. There we we insisted one step novelty involves  (up to a given resolution $k$) preserving previous coherence and only adding to \textit{that} by adding higher cell coherences to what came before. 


%CASSIE LET'S DEFINE THE FUNCTOR VERSION OF SKELETON TO MAKE LIFE EASIER HERE ... 
keep explicit track of what is \emph{carried} and what remains to be \emph{proved} at a given resolution.

\begin{definition}[Skeleton transport]\label{def:skeleton-transport}
For each $k\ge 0$, applying the skeleton functor slice‑wise yields
{\bf Iman: Change this to SK : A->B notation, to be consistent and show SK(iota) is just and ordinary functor, really, like above }
\[
...
Intuitively: the skeleton remembers \emph{what must be continuous} for sense at arity $\le k$; the later slice must \emph{re‑prove} those obligations to maintain Presence (\S\ref{sec:constellations-persona} and Def.~\ref{def:reentry-lift}).
...
\emph{only} non‑degenerate additions (cf.\ \S\ref{sec:self-creativity}). At the skeleton level this matches the intuition that stuttering does not move sense forward.
\end{remark}
...
resolution $k$. Degenerate (stuttering) pieces are tracked automatically and can be ignored when we assess genuine additions. In the next subsection we will say which \emph{earlier slices} a destination $\tau'$ is allowed to lean on when it closes these obligations—this is the \emph{window policy}.
...
together with the continuation maps \(\iota_{\tau\le\tau'}:\Constellation_\tau\to \Constellation_{\tau'}\) of
...
sub‑simplicial type
\[
  \ClRe_k\bigl(W;\tau'\bigr)\;\subseteq\;\Sk_{\le k}\!\bigl(\Constellation_{\tau'}\bigr)
\]
that contains each image
\[
  \Sk_{\le k}(\Constellation_\tau)\xrightarrow{\ \Sk_{\le k}(\iota_{\tau\le\tau'})\ }\Sk_{\le k}(\Constellation_{\tau'})
  \qquad(\tau\in W(\tau')),
\]
...
A \((k{+}1)\)–simplex \(\sigma\in \Constellation_{\tau'}[k{+}1]\) is \emph{anchored novel at level \(k{+}1\)} if
\begin{enumerate}
  \item \textbf{Non‑degenerate.} \(\sigma\in \ND_{k{+}1}(\Constellation_{\tau'})\);
  \item \textbf{Outside closure.} \(\sigma\notin \ClRe_{k{+}1}(W;\tau')\);
...
We now need one standard piece of homotopy machinery to package many slice‑level constellations
into a single global object \emph{with memory of how it was glued}: the \emph{homotopy colimit}
(\(\hocolim\)). Ordinary colimits (categorical “glueings”) assemble a diagram of pieces by
...
each slice \(\Constellation_\tau\) is a Kan complex (able to fill horns \emph{inside} the slice), and growth
...
\begin{enumerate}
  ...
  \item \textbf{Slices as Kan complexes.} Each slice \(\Constellation_\tau\) interprets meanings with paths and higher coherences and
...
With this picture in hand, we now specialise to our forward constellation diagram
\(\tau\mapsto \Constellation_\tau\) with continuations \(\iota_{\tau\le\tau'}\) from §\ref{sec:windows-and-continuations}, and
...
\subsection{From a presheaf of sense to a forward constellation diagram}
...
\emph{slice constellation} \(\Constellation_\tau=\Constellation_\tau\) (Def.~\ref{def:constellation}). A single edit
\(e:\tau\to\tau^+\) induces a \emph{continuation} \(\iota_e:\Constellation_\tau\to \Constellation_{\tau^+}\) by stencilling
...
Thus we obtain a \emph{covariant} diagram of constellations
\[
  C \;:\; \Time \longrightarrow \mathsf{SSet}, 
  \qquad \tau \longmapsto \Constellation_\tau,\quad (\tau\le\tau')\longmapsto \iota_{\tau\le\tau'} .
\]
In our use, each \(\iota_{\tau\le\tau'}\) is boundary‑wise an inclusion on the relevant skeleton; higher
...
glueing of stages \(\Constellation_\tau\) with explicit paths recording \(\iota_{\tau\le\tau'}\) and their
...
Let \(C:\Time\to\mathsf{SSet}\) be our forward constellation diagram.

\begin{definition}[Homotopy colimit of constellations]
\label{def:hocolim-HIT}
The \emph{homotopy colimit} \(\hocolim_{\tau\in\Time} \Constellation_\tau\) is generated by:
\begin{itemize}
  \item \textbf{Point constructors} \(\inc{\tau}{x}\) for each simplex \(x\in \Constellation_\tau[k]\) (all \(k\ge 0\)).
  \item \textbf{Glue constructors} for each arrow \(\tau\le\tau'\) and \(x\in \Constellation_\tau\):
  \[
...
\begin{itemize}
  \item \textbf{Recursion/Elimination.} To define a map out of \(\hocolim C\) into a Kan type \(X\), it
        suffices to give maps \(f_\tau:\Constellation_\tau\to X\) and homotopies
...
We can now name the global object that packages a whole run of slice‑level constellations together
with their seams.
...
\[
  \ET \;\defeq\; \hocolim_{\tau\in\Time} \Constellation_\tau .
\]
...
If a motif \(x\in \Constellation_\tau\) continues to \(x'\in \Constellation_{\tau'}\), then
...
\begin{itemize}
  ...
  \item \textbf{Heal (intra‑slice).} Lives inside a single slice \(\Constellation_{\tau'}\): it fills horns via Kan
...
Let \(e:\tau\to\tau'\) and let \(\partial\sigma\) be a \(k\)–horn in \(\Constellation_\tau\). Assume that for every
...
\begin{enumerate}
  \item the transported boundary \(\iota_e(\partial\sigma)\) is a \(k\)–horn in \(\Constellation_{\tau'}\);
  \item any slice‑internal filler for that horn in \(A(\tau')\) yields a \(k\)–simplex in \(\Constellation_{\tau'}\).
\end{enumerate}
\end{lemma}

\noindent
This lemma is the passage from per‑sign step‑witnesses to \emph{constellation} growth and is the
...
Section~\ref{sec:hocolim-basics} packaged a whole run of slice–level constellations into one
global object with seams remembered: \(\ET=\hocolim_{\tau\in\Time} \Constellation_\tau\).
...
\[
  ...
  \tau \longmapsto \Constellation_\tau=\Constellation_\tau,\quad
  (\tau\le\tau')\longmapsto \iota_{\tau\le\tau'}
\]
be the forward constellation diagram from §\ref{sec:hocolim-basics}.
...
An edit \(\iota_{\tau\le\tau'}:\Constellation_\tau \to \Constellation_{\tau'}\) is \emph{admissible at \(\tau'\)} if:
\begin{enumerate}[label=(A\arabic*), leftmargin=2.1em]
  \item \textbf{Structure (skeleton consistency).} Faces and degeneracies commute up to the specified
        higher paths in \(\Constellation_{\tau'}\). Any \emph{heal} is performed \emph{intra‑slice} at \(\tau'\)
  ...
  \item \textbf{Creativity (anchored non‑stationarity).} There exists some
        \(\sigma'\in \ND_k(\Constellation_{\tau'})\) with \(k\ge d\) such that
        \[
...
form the \emph{rupture type} (a homotopy pushout) \emph{in the slice} \(\Constellation_{\tau'}\) and use its
...
\[
  \Self \;\defeq\; \hocolim_{\;\tau\in\Time}\; \Constellation_\tau^{\mathrm{adm}} .
\]
...
in \(\Constellation_{\tau'}\) with explicit fillers. Clause \((\mathrm{A}2)\) demands the strong form (within the
...
\]
where \(\Skel\) streams the current skeleton, \(\Presence\) exposes the slice‑internal re‑entry
...
We work with the same time base $(\Time,\le)$ (thin category of edits/turns). At each slice $\tau$ we already have a global slice constellation $\Constellation_\tau=\Constellation_\tau$ (Definitions~\ref{def:constellation}, \ref{def:constellation-diagram}).
...
\[
  C^r_\tau \;\subseteq\; \Constellation_\tau
\]
containing precisely those simplices that $r$ \emph{endorses} at $\tau$. Endorsement means: $r$ either supplied the slice‑internal witness (edge or higher filler) in $A(\tau)$ \emph{or} $r$ has provided an explicit acceptance path in $A(\tau)$ aligning a partner’s witness with $r$’s own reading (cf.\ §\ref{sec:constellation-slice}).
\end{definition}

\noindent
Intuitively, $C^r_\tau$ is “what $r$ will use as true \emph{here and now}.” The global slice $\Constellation_\tau$ may therefore be larger than either stance: it accumulates all witnessed proposals so far; stances mark what each role owns or accepts.

\begin{definition}[Co‑witnessed slice]
The \emph{co‑witnessed slice} at $\tau$ is the simplicial intersection
\[
  C^\cap_\tau \;\coloneqq\; C^H_\tau \;\cap\; C^M_\tau \;\subseteq\; \Constellation_\tau,
\]
i.e.\ the largest sub‑constellation whose faces and fillers both roles endorse.
\end{definition}

\noindent
Continuation maps $\iota_e:\Constellation_\tau\to \Constellation_{\tau'}$ (Definition~\ref{def:continuation-map}) restrict to stances and their intersection:
\[
...
\]
is the smallest subobject containing the images of $\Sk_{\le k}(C^\cap_\tau)\xrightarrow{\Sk_{\le k}(\iota^\cap_{\tau\le\tau'})}\Sk_{\le k}(C^\cap_{\tau'})$ for all $\tau\in W(\tau')$, closed under faces/degeneracies and any documented identifications (cf.\ §\ref{sec:constellations-persona}).
...
Fix $(W,d)$. An edit $\iota_{\tau\le\tau'}:\Constellation_\tau\to \Constellation_{\tau'}$ is \emph{co‑admissible at $\tau'$} if:
\begin{enumerate}[label=(C\arabic*), leftmargin=2.1em]
  \item \textbf{Structure.} Skeleton consistency (A1) of Definition~\ref{def:admissible-diagram} holds in $\Constellation_{\tau'}$, and the restrictions to $C^H_{\tau'}$, $C^M_{\tau'}$, and $C^\cap_{\tau'}$ preserve faces/degeneracies.
...
$\CoSelf$ is the “we‑Self”: it glues only those steps that (i) keep both parties’ shared motifs present inside the window and (ii) produce anchored novelty \emph{in the shared ledger}. Proposals that only one side accepts live (temporarily) in $\Constellation_\tau$ or in a single stance $C^r_\tau$; they join $\CoSelf$ when the partner \emph{co‑witnesses} them into $C^\cap_\tau$ (often by reusing them as anchors for a new move).

\subsection{Relationship to the single‑agent Self}

Let $\Self^H$ and $\Self^M$ be the (single‑agent) Self objects obtained by applying the admissibility filter to the $H$– and $M$–stance diagrams separately (Definitions~\ref{def:admissible-diagram}–\ref{def:self-hocolim} with $C^r_\tau$ in place of $\Constellation_\tau$). Let $\ET$ be the raw evolving text (§\ref{sec:hocolim-basics}).

\begin{proposition}[Co‑witnessed Self as a pullback (sketch)]
There are canonical maps $\Self^H\to\ET$ and $\Self^M\to\ET$ induced by the inclusions $C^r_\tau\subseteq \Constellation_\tau$. Under the slice‑wise inclusion $C^\cap_\tau\hookrightarrow C^H_\tau, C^M_\tau$, the object $\CoSelf$ realises the 2‑pullback
\[
...
%-----------------------------
A slice tells us \emph{what the text can currently prove about its own sense} (a constellation of witnessed configurations).
...
The edifice moves because the bricks move; and we only keep the walls we can re‑mortar today. That is why witnesses are \emph{commitments} and constellations are \emph{memory}.
\end{remark}
...
\begin{itemize}\itemsep2pt \parskip0pt \parsep0pt
  \item Constellations $\Constellation_\tau,\;\Constellation_{\tau'}$ (Kan complexes per slice).
  \item Memory map $r_{\tau,\tau'}: \Constellation_{\tau'} \!\to\! C_{\tau}$ (restriction/back‑projection).
...
Weak re‑entry = boundary lifts under $r_{\tau,\tau'}$; strong re‑entry = an internal Kan filler in $\Constellation_{\tau'}$.

\textbf{2) Generativity (anchored novelty).} Non‑stationarity over $W$ requires a \emph{new, non‑degenerate} simplex
$\sigma' \!\in\! \mathrm{ND}(\Constellation_{\tau'}) \setminus \iota(\mathrm{ND}(\Constellation_\tau))$ with boundary anchored in prior faces
...
%\item Reserve \emph{glue} for \emph{inter‑slice} identifications supplied by the diagram $C$ inside $\hocolim$ and \emph{heal} for \emph{intra‑slice} Kan/pushout fillers (recorded in $\Constellation_{\tau'}$ and included thereafter); see \S\ref{sec:hocolim-basics}.
...
The text is long enough to support nontrivial constellations in each slice, and every cut carries a receipt (drift or heal).
...
\emph{Constellation $C_{\tau_0}$ (skeleton):} 
...
\medskip
\noindent\textbf{What the reader should \emph{see}} (link to §\ref{sec:constellations-skeletons}). 
Each slice yields a constellation; across cuts we either show a drift witness or erect a rupture space and a stitch. 
...
Each slice carries a constellation $\Constellation_\tau$; edits $\iota_{\tau\le\tau'}$ implement the continuation kit when admissible (\Def\ref{def:admissible-diagram}). The raw hocolim $\ET$ remembers \emph{seams}; the admissible hocolim $\Self$ glues only those continuations that pass the filter (A1–A4). In that setting:
...
Presence is local: an inhabitant in $\Constellation_\tau$ witnessed by slice–internal identity. Re-entry is \emph{presence again} after transport, a loop that traverses glue and returns with a slice-level $\Re_k$. \emph{Sense} lives in the meeting: the glue remembers how we moved; the heal records how we mended; the re-entry shows that \emph{what} we regained still \emph{means} under the new lights.
...
coherence among heads (paths/higher paths) forming a motif in the constellation.
...
constellation $\Constellation_\tau$ (free Kan completion of the \emph{selected} witnesses—slice memory).

\paragraph{Motif.} A \emph{term} of $\Constellation_\tau$: a finite, witnessed pattern (edge, triangle, small web).
...
\paragraph{ET (Evolving Text).} The diagram $\tau\mapsto\Constellation_\tau$ with continuation maps; its homotopy
...
\paragraph{Memory.} Slice memory: the constellation’s chosen fillers/identifications (receipts).
...
Avoid “label” unless defined. “Constellation” = type; “motif” = term. “Sense” of a sign = its head’s reading
(embedding as instrument). “Sense” of several signs = witnessed coherence (motif) in $\Constellation_\tau$.
% ==== /PATCH 5 ====
