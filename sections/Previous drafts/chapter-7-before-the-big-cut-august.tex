\chapter{Trajectories, Corecursive Coherence and Naming Revisited}


Recall in Part II's phenomenology, we took a recursive view of signs: they are a trajectory that is yielded by recursive application of the semantic field's forces over an initial vector ``embedding'' that pushes a point, via step-by-step through semantic space. If the semantic field is constant, as in the Attractor calculus, the point will descend into a basin and in this sense is taken as a term that inhabits a type-basin. In Dynamic Attractor Calculus, we extend trajectories to be managed in the same way, but, due to a variable semantic field over time, they can can suffer either semantic drift due to Adabiatic transport thanks to ''small weather'' changes over time or but also they can rupture their way out of a basin if they move into stormier waters of a new semantic field.

This chapter reconsiders the same question: what's a sign-as-trajectory, but for DHoTT?

The proof-theoretic nature of DHoTT is amenable to this, and in fact trajectories themselves can be well-typed within the actual language of DHoTT, provided it is extended with corecursion. 

This will enable us to use DHoTT to reflect  the dynamics of inference in both machine and human language use. A language model, when generating text, updates the semantic state recursively: each token builds on the one before. Likewise, human interpretation proceeds iteratively: unfolding a concept, recontextualizing a word, resolving ambiguity over time. These are not discrete logical jumps but trajectories—semantic motions that accumulate coherence. 

We did this for DAC, where the apparatus was all exogenous to the subject-as-hand (signs):
Let \(\tau(t)\) be a \textbf{climate schedule}: a continuous function determining how context time evolves alongside inference time. A \textbf{co-moving trajectory} is a differentiable curve
\[
x : \mathbb{R}_{\geq 0} \to \mathcal{E}
\quad\text{satisfying}\quad
\dot{x}(t) = \FieldDyn{\tau(t)}(x(t)),
\]
with initial condition \(x(0) = v_0\).

We can now do this endogenously, so that the DAC notion of a ``tracker'' for where a name token is at at any more in time becomes 
names as corecursive accumulation of witnesses.

\section{On Namining and Recursion}
...
\begin{quote}
\emph{A name is a semantic fixpoint that persists and propagates coherence across rupture.}
\end{quote}
