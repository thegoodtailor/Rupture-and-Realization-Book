...
\]
Small $d_\angle$ (equivalently, small $\delta_\angle$) indicates similar \emph{use in context}. Two occurrences are \emph{neighbours} when $d_\angle(\hat e_{t_i},\hat e_{t_j})\le r$ for a chosen scale $r>0$.
...
At the scale of a single text, dense regions of compatible use are summarised as \emph{basins}. With the angular metric, a basin centred at a representative direction $\mu_j$ with radius $r_j$ is the spherical cap
\[
B_j \;:=\; \{\,x\in S^{d-1} \mid d_\angle(x,\mu_j) < r_j \,\}.
\]
We may still speak informally of “balls,” but—unless noted otherwise—\emph{radii are angular} (radians) and the sets are open caps on $S^{d-1}$.

\emph{Implementation note.} In computation we often test membership with $\le$ and apply an $\varepsilon$-thickening; this does not affect the homotopy type at the scales used.
...
B_j \;=\; \{\,x\in S^{d-1} \mid d_\angle(x,\mu_j) < r_j \,\},
\]
where $\mu_j$ is a mean direction/Fréchet mean on $S^{d-1}$ and $r_j>0$ is a robust within-cluster quantile of geodesic radii. Each $B_j$ is a \textbf{basin}: a region where the text is currently realising a compatible reading.
...
As above, we model basins as spherical caps on $S^{d-1}$ with $r_j<\pi/2$. Finite intersections of such caps are geodesically convex and thus contractible, so the good-cover hypothesis for the Nerve Lemma applies unchanged.

To guarantee this, we enforce a hard clamp: $r_j \leftarrow \min\{r_j,\, \pi/2 - \varepsilon\}$ for small $\varepsilon > 0$. In practice, typical within-cluster radii from distributional clustering are well below $\pi/2$, so this clamp rarely activates.

\emph{Open vs.\ closed in implementation.} Theoretically our caps are open ($d_\angle < r_j$); computationally we often test $\le$ for numerical stability. This closed/thickened proxy does not affect homotopy type at the scales used, since the perturbation is small relative to inter-basin distances.
...
$r_j<\pi/2$ so caps are geodesically convex), the standard tool is the \Cech{}
nerve of the cover.
...
\begin{definition}[Token graph]
\label{def:token-graph}
The graph $G_{\mathrm{tok}}(T,\Ucov)$ has vertices $T$ and
\[
\{t_i,t_j\}\in E
\iff
...
\begin{tcolorbox}[colback=gray!5, colframe=gray!50, title=\textbf{Map of Objects}]
We work with two levels of structure built from the same incidence relation $R \subseteq T \times J$:

\medskip
\textbf{Cover level (basins as vertices):}
\begin{itemize}
\item $N_{\mathrm{geom}}(\Ucov)$ --- the geometric \v{C}ech nerve: $[j_0,\dots,j_k]$ is a simplex iff $\bigcap_s B_{j_s} \neq \varnothing$
\item $N_{\mathrm{obs}}(\Ucov)$ --- the observed nerve: $[j_0,\dots,j_k]$ is a simplex iff some token inhabits $\bigcap_s B_{j_s}$
\item $A_{\mathrm{basin}} := \Ex^\infty N_{\mathrm{obs}}(\Ucov)$ --- Kan replacement at cover level
\item Note: $N_{\mathrm{obs}} \subseteq N_{\mathrm{geom}}$; equality holds in the dense-sampling regime. The Nerve Lemma applies to $N_{\mathrm{geom}}$; the \emph{shape} of sense lives here.
\end{itemize}

\textbf{Token level (tokens as vertices):}
\begin{itemize}
\item $K_{\Tok}(R)$ --- token Dowker complex: simplex iff tokens \textbf{share a basin} (\emph{strong coherence})
\item $G_{\mathrm{tok}}$ --- \v{C}ech pullback graph: edge iff tokens' \textbf{basins overlap} (\emph{general coherence})
\item $\ET(\tau) := \Ex^\infty K_{\Tok}(R)$ --- the HoTT base for token-level reasoning
\end{itemize}

\medskip
\textbf{Two grades of token coherence:}
\begin{itemize}
\item \emph{Strong}: tokens $a,b$ share a basin ($\mathrm{mem}(a) \cap \mathrm{mem}(b) \neq \varnothing$) --- direct edge in $K_{\Tok}(R)$
\item \emph{General}: tokens $a,b$ have overlapping basins --- connected via path in $G_{\mathrm{tok}}$
\end{itemize}
Example: ``cat'' and ``feline'' in the same basin = strong coherence (near-identity of use). ``Feline'' and ``Leghato'' (a particular cat) in overlapping but distinct basins = general coherence (compatible but distinguishable).

\medskip
\textbf{Connection via Dowker duality:} By Dowker's theorem, $|K_{\Tok}(R)| \simeq |K_J(R)|$, and $K_J(R) \subseteq N_{\mathrm{obs}}(\Ucov)$. The token-level space $\ET(\tau)$ and the cover-level space $A_{\mathrm{basin}}$ are thus related: shape arguments use the cover level (Nerve Lemma); HoTT reasoning uses the token level (where individual mentions live).

\medskip
The \textbf{radius $r_j$ is a dial}: larger radii produce more overlaps, more edges, coarser coherence. The choice of radius encodes a theory of how tightly tokens must cluster to count as ``the same kind of thing.''
\end{tcolorbox}

\begin{cassiebox}\label{box:phenomenology}
\textbf{Phenomenology.}
At cover level, a Čech simplex is a witnessed multi-overlap of basins---places where different regions of sense meet.
At token level, $K_{\Tok}(R)$ captures \emph{strong coherence} (tokens that cohabit one region), while
$G_{\mathrm{tok}}$ captures \emph{general coherence} (tokens whose regions bridge). The latter
...
\]
\emph{Convention:} $\check{C}(P,r)$ uses balls of radius $r$; $\VR(P,\epsilon)$ uses pairwise distance threshold $\epsilon$.
...
and pick $r$ in the range $s/2 < r < s/\sqrt{3}$. Čech at radius $r$ has edges (pairwise intersections exist) but no $2$-simplex (the triple
intersection is empty; the circumradius $s/\sqrt{3} > r$), so a loop survives. VR at
scale $2r > s$ completes the triangle by clique, killing the loop prematurely.

This is not a pathology but a feature: Čech respects \emph{joint} presence, while VR only checks pairwise closeness.
...
\begin{definition}[Canonical fibrant replacement (cover level)]
At the cover level, set
\[
  A_{\mathrm{basin}} \;:=\; \Ex^\infty N(\Ucov) \;\in\; \mathbf{sSet}_{\Kan}.
\]
Then $A_{\mathrm{basin}}$ is Kan: for every horn $\Lambda^n_i\!\to\!A_{\mathrm{basin}}$ there exists a filler
$\Delta^n\!\to\!A_{\mathrm{basin}}$. The unit $\eta_\infty\colon N(\Ucov)\to A_{\mathrm{basin}}$ is a weak
equivalence. Thus passing to $A_{\mathrm{basin}}$ preserves homotopy type while guaranteeing
an internal, compositional path calculus.

At the token level, we similarly define $\ET(\tau) := \Ex^\infty K_{\Tok}(R)$, which is the base for HoTT reasoning over tokens (see \S\ref{sec:hott-logic-final}).
\end{definition}
...
\begin{figure}[htbp]
  ...
  \emph{Solid} lines: Čech 1-skeleton (witnessed co-presence in a shared basin).  
  \emph{Dashed}: edges visible only in $\ET(\tau)=\Ex^\infty K_{\Tok}(R)$ when
  a coherent composite path exists via the basin graph. Dashed
  ...
  \begin{multline*}
    ...
    \prod_{\text{edges }(a,b)}
      \frac{|W_{ab}|}
           {\min\{|W_a|,|W_b|\}},
  \end{multline*}
  where $W_{ab}$ denotes the set of witness tokens inhabiting both $B_a$ and $B_b$, and $|W_j|$ the token count in basin $j$. Retain only $w(u,v)\ge \theta$.
\end{enumerate}
...
\begin{itemize}
\item \emph{Type} $\leftrightarrow$ the Kan fibrant token complex $\ET(\tau)=\Ex^\infty K_{\Tok}(R)$;
...
Let $T$ be the set of \emph{tokens} in the slice and $\mathcal U=\{B_j\mid j\in J\}$ the basin cover from \S\ref{sec:cover}. 
Consider the incidence relation $R\subseteq T\times J$ given by $(t,j)\in R\iff \hat{e}_t\in B_j$.
We take as base the \emph{token Dowker complex} $K_{\Tok}(R)$: a finite set $\sigma\subseteq T$ is a simplex iff the tokens in $\sigma$ share at least one basin, i.e.\ $\bigcap_{t\in\sigma} R(t,-)\neq\varnothing$. This captures \emph{strong coherence}: tokens that literally cohabit a region of sense.

By Dowker's theorem, $|K_{\Tok}(R)| \simeq |K_J(R)|$, where $K_J(R)$ is the cover Dowker complex. Note that $K_J(R)$ is a \emph{witnessed subcomplex} of the full \v{C}ech nerve $N_{\mathrm{geom}}(\Ucov)$: a geometric multi-overlap can exist without any single token witnessing it. Our HoTT semantics operates at the token level, ensuring every path has empirical grounding in observed tokens.

\begin{remark}[Design choice]
\label{rem:design-choice}
The Čech-based instrumentation is one principled choice encoding a distributional view of meaning: overlapping contextual regions imply semantic compatibility. The Dynamic Homotopy framework of Chapter~\ref{chap:evolving-text-as-presheaf} is agnostic about this choice---other instrumentations (e.g., $k$-nearest neighbor graphs, context window co-occurrence, attention-derived adjacency) would yield different but equally valid instantiations of the Generic Dynamic Schema. We develop this one because (i) it grounds coherence in witnessed geometric structure, and (ii) it connects naturally to the TDA machinery for bars in Chapter~\ref{chap:bars}.
\end{remark}

\paragraph{Kan replacement and path calculus.}
We pass to a Kan fibrant replacement $\ET(\tau) := \Ex^\infty K_{\Tok}(R)\in\mathbf{sSet}_{\mathrm{Kan}}$. 
Points $x:\ET(\tau)$ are tokens. A path $p:\Id_{\ET(\tau)}(x,y)$ is a \emph{coherence path}: a composition of observed co‑presences (solid edges) and their Kan fillers. 
...
\begin{tcolorbox}[colback=blue!3, colframe=blue!40, title=\textbf{Run Specification (Chapter 2 Demo)}]
\begin{tabular}{ll}
\textbf{Encoder:} & DeBERTa-v3-base, layer 9 (768 dimensions) \\
\textbf{Tokenizer:} & DeBERTa WordPiece (subwords kept as tokens) \\
\textbf{Token count:} & $n = 847$ tokens in slice \\
\textbf{Clustering:} & Spherical $k$-means with cosine distance, $k=12$ \\
\textbf{Basin radii:} & $r_j = 0.85$-quantile of within-cluster angular distances \\
\textbf{Witness selection:} & All content tokens in $B_i \cap B_j$ (stopwords retained) \\
\end{tabular}
\end{tcolorbox}

\emph{Note:} Tokens with $\mathrm{mem}(x) = \varnothing$ in Table~\ref{tab:mem-probes} are outliers---points outside all basin caps at the chosen radii. They are retained in the token set but contribute no edges to $K_{\Tok}(R)$.
...
Fix the probe set used in Fig.~\ref{fig:cech-tokens}. 
...
{ \ap{f}{q\circ p}\ \simeq\ \ap{f}{q}\circ \ap{f}{p} }
...
{ p:x{=}y \quad q:y{=}z }
{ \transportargs{B}{q\circ p}\ \simeq\ \transportargs{B}{q}\circ \transportargs{B}{p} }
...
In Chapter~3, each time slice $\tau$ yields its own cover $\Ucov_\tau$ on the sphere, its own nerve $N(\Ucov_\tau)$, and its own Kan replacement $\ET(\tau)$. This chapter has built the single-slice version of that machinery. The only new ingredient in Chapter~3 is \emph{time}: how we compare slices, define carry and rupture between them, and track the evolution of sense as a presheaf over a category of moments. The static geometry becomes dynamic; the snapshot learns to breathe.
