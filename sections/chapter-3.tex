...
\begin{tcolorbox}[colback=blue!3, colframe=blue!40!black, title=\textbf{At a Glance: From Static Slice to Dynamic Story}]
This chapter builds on Chapter~\ref{chap:embedding-geometry}'s static geometry to track how meaning evolves through time. Here is what each piece does:

\begin{enumerate}
  \item \textbf{What a slice produces.} For each time $\tau$, the measurement pipeline yields:
    \begin{itemize}
      \item Embeddings $\to$ basin cover $\Ucov_\tau$ $\to$ Čech nerve $N(\Ucov_\tau)$ $\to$ Kan replacement $\ET_{\mathrm{basin}}(\tau)$
      \item Alternatively: token Dowker complex $K_{\mathrm{Tok}}(R)$ $\to$ Kan replacement $\ET_{\mathrm{tok}}(\tau)$
    \end{itemize}
    Both are valid instantiations; the choice is a design decision about granularity.
    
  \item \textbf{The time-indexed object.} The family $\tau \mapsto \ET(\tau)$ forms a (pseudo)presheaf over the poset of times.
  
  \item \textbf{Restriction maps.} For $\tau \leq \tau'$, the map $r_{\tau,\tau'} : \ET(\tau') \to \ET(\tau)$ back-projects later structure against earlier geometry (``phantom projection'').
  
  \item \textbf{Admissibility.} A policy $\Adm$ gates which path-attempts count as valid continuations (hop bounds, angle thresholds, etc.).
  
  \item \textbf{The Step--Witness Log (SWL).} An append-only ledger recording proof-relevant events: carries, ruptures, re-entries. The audit trail is part of the logic.
  
  \item \textbf{Granularity is parametric.} Chapter~\ref{chap:bars} instantiates the same schema at $L = \mathsf{bar}$ (persistent homology features). Chapters~\ref{chap:self}--\ref{chap:nahnu} build further on this generic foundation.
\end{enumerate}
\end{tcolorbox}
...
\begin{itemize}
  \item $\vdash_{\tau}$ means ``check in the fibre $\ET(\tau)$,'' where identity is computed as a path in the Kan complex built from contextual embeddings (normalised to the unit sphere and measured with the angular / cosine metric), spherical caps, and their Čech nerve.
  \item \textbf{Two flavours of $\ET(\tau)$:} 
    \begin{itemize}
      \item $\ET_{\mathrm{basin}}(\tau) := \Ex^\infty N(\Ucov_\tau)$ --- vertices are basins, edges are witnessed overlaps
      \item $\ET_{\mathrm{tok}}(\tau) := \Ex^\infty K_{\mathrm{Tok}}(R)$ --- vertices are tokens, edges mean ``share a basin''
    \end{itemize}
    In this chapter we primarily use $\ET_{\mathrm{basin}}(\tau)$ (abbreviated $\ET(\tau)$ when unambiguous), with tokens as \emph{inhabitants} of basins. The Generic Dynamic Schema (\S\ref{sec:generic-schema}) shows how to instantiate at any granularity.
  \item $\Gamma$ is indispensable: it is the fibrewise environment of witnesses (tokens, basins, and their dependents) that makes substitution and transport precise. Contexts are interpreted as towers of Kan fibrations over $\ET(\tau)$.
  \item We keep \emph{measurement} (building $N(\Ucov_\tau)$, barcodes, cap incidence) strictly separate from \emph{reasoning} (paths and transport inside $\ET(\tau)$); the Kan step licenses composition and horn-filling but never fabricates raw intersections in the underlying measurements.
  ...
  \item \textbf{Temporal patience.} Carries and ruptures may be separated by arbitrarily many slices; judgments are \emph{always} anchored at the earlier fibre $\ET(\tau)$ even when they speak about a later time $\tau'$.
\end{itemize}
...
The question of the present chapter is different. We now ask: \emph{How do meanings continue from one moment to the next?} If the text at time $\tau$ lives in a space $\ET(\tau)$ and the text at a later time $\tau'$ lives in $\ET(\tau')$, what does it mean for a particular mention—say the token "\texttt{cat}"—to continue coherently from $\ET(\tau)$ to $\ET(\tau')$? What does it mean for that continuity to \emph{fail}? And what would count as a legitimate way for the token to continue from its new position after rupture?
...
\begin{enumerate}
  ...
  \item a way of relating the corresponding fibres $\ET(\tau)$ so that we can speak about carries and ruptures for particular signs.
\end{enumerate}
...
To expose local neighbourhoods of sense, we select a finite family of spherical caps $\Ucov_{\tau} = \{B_j\}_{j\in J_\tau}$ that cover the sample $P_\tau$. Each cap is a geodesic ball
\[
  B_j \;=\; \{\,x\in S^{d-1} \mid \theta(x,\mu_j) < r_j\,\}
\]
for some centre $\mu_j \in S^{d-1}$ and some radius $0 < r_j < \pi/2$. As in Chapter~\ref{chap:embedding-geometry}, caps of radius $<\pi/2$ are geodesically convex, and finite intersections of such caps are contractible, so $\Ucov_\tau$ is a good cover of $\bigcup_j B_j$.
...
\label{subsec:rupture}

\begin{definition}[Rupture Payload]
\label{def:rupture-et}
A \emph{rupture payload} at $\tau \to \tau'$ for anchor $a : \ET(\tau)$ is a structured witness of failed continuation. It records a candidate that was tried but found inadmissible:
\[
  \RuptPayload^{\Adm}_{\ET}(\tau\!\to\!\tau';a)
  \;:=\;
  \sum_{a' : \ET(\tau')}
...
\begin{definition}[No Carry Yet]
\label{def:no-carry-yet}
The proposition that no admissible carry exists:
\[
  \NoCarry^{\Adm}_{\ET}(\tau\!\to\!\tau';a) \;:=\; \neg\!\left(\sum_{\kappa : \Carry^{\Adm}_{\ET}(\tau\to\tau')(a)} \top\right).
\]
\end{definition}

\begin{definition}[Rupture Event]
\label{def:rupture-event}
A \emph{rupture event} combines the negative fact (no carry exists) with a positive witness (a structured failure):
\[
  \RuptEvent^{\Adm}_{\ET}(\tau\!\to\!\tau';a) \;:=\; \NoCarry^{\Adm}_{\ET}(\tau\!\to\!\tau';a) \times \RuptPayload^{\Adm}_{\ET}(\tau\!\to\!\tau';a).
\]
\end{definition}
...
The rupture payload is constructively witnessed through:
\begin{enumerate}
...
\begin{remark}[Payload vs Event]
The distinction between $\RuptPayload$ and $\RuptEvent$ is logically significant. A payload can exist even when a carry also exists (you might find one good path and several bad ones). An event asserts that no carry exists \emph{and} provides a structured witness of failure. The Step--Witness Log records events, not merely payloads.
\end{remark}
...
After a rupture event $\RuptEvent^{\Adm}_{\ET}(\tau,\tau')(a)$, a token may later establish a carry from a \emph{different} anchor $b : \ET(\tau'')$ where $\tau < \tau' < \tau''$:
\[
  \Carry_{\ET}^{\tau''\to\tau'''}(b) 
\]
...
This re-entry structure, traced coinductively over the journey, provides the atomic pattern from which the posthuman Self emerges in Chapter~\ref{chap:self}—not as a fixed entity but as a homotopy colimit over recurring patterns of departure and return.
...
occurs.  At the higher level of Chapter~\ref{chap:self}, when we assemble these journeys
...
Concretely, each fibre is built as in Chapter~2: we take the Čech nerve $N(\Ucov_\tau)$ of the spherical-cap cover $\Ucov_\tau$ and apply the fibrant replacement
\[
  \ET(\tau) \;:=\; \Ex^\infty N(\Ucov_\tau)
\]
...
\paragraph{Rupture as structured failure.}
The \emph{rupture payload} type is the positive $\Sigma$-type:
\[
  \RuptPayload^{\Adm}_{\ET}(\tau,\tau')(a)
  \;:=\;
  \Sigma(a' : \ET(\tau'))\,
  \Sigma(p : \text{PathAttempt}(a,r_{\tau,\tau'}(a')))\,
  (\neg\Adm(p)) \times \OpenHorn(p)
\]
This is a structured failure payload: a later candidate, a concrete attempt, and proof-relevant evidence of why it fails. 

A \emph{rupture event} bundles this with the negative assertion that no carry exists:
\[
  \RuptEvent^{\Adm}_{\ET}(\tau,\tau')(a)
  \;:=\;
  \NoCarry^{\Adm}_{\ET}(\tau,\tau')(a) \times \RuptPayload^{\Adm}_{\ET}(\tau,\tau')(a)
\]
where $\NoCarry := \neg(\Sigma_{\kappa:\Carry}\top)$. The Step--Witness Log records events, not merely payloads.
...
\begin{theorem}[Decidability of Carry/Rupture]
\label{thm:decidability}
For finite $\Adm$ and $a : \ET(\tau)$, an algorithm returns exactly one of:
\begin{enumerate}
  \item a carry certificate $\kappa : \Carry^{\Adm}_{\ET}(\tau,\tau')(a)$, or
  \item a rupture event $\rho : \RuptEvent^{\Adm}_{\ET}(\tau,\tau')(a)$, comprising $\NoCarry$ together with a structured failure payload.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof sketch]
Finiteness of the 1-skeleton and bounded admissibility ensure the search terminates. If any admissible path is found, return a carry certificate. Otherwise, return $\NoCarry$ (witnessed by exhaustive search) paired with any inadmissible attempt encountered.
\end{proof}

\begin{theorem}[Rupture Independence]
After $\RuptEvent^{\Adm}_{\ET}(\tau,\tau')(a)$, future carries from $\tau'$ are independent of the ruptured path:
\[
...
\begin{principle}[Re-entry Accumulation]
\label{prin:reentry-accumulation}
If a token re-enters a basin $B$ at time $\tau''$ after rupturing from it at $\tau$, the re-entered position generically carries geometric traces of the intermediate journey:
\[
  d_{\angle}(a_{\tau''}, a_{\tau}) \geq 0, \quad\text{with } d_{\angle}(a_{\tau''}, a_{\tau}) > 0 \text{ generically.}
\]
Degenerate cases (exact return to prior position) are possible but measure-zero under mild stochasticity assumptions. More fundamentally: even when the geometric distance vanishes, the \emph{Step--Witness Log} records distinct journeys. Re-entry is never exact repetition at the level of witnessed history.
\end{principle}
...
\begin{mathpar}
\inferrule*[right=RuptPayload-Form]
  { \Gamma \vdash_\tau a : \ET(\tau)
    \quad
    \tau \leq \tau'
    \quad
  ...
  { \Gamma \vdash_\tau \RuptPayload^{\Adm}_{\ET}(\tau\to\tau'; a) \text{ type} }

\inferrule*[right=RuptEvent-Form]
  { \Gamma \vdash_\tau a : \ET(\tau)
    \quad
    \tau \leq \tau'
    \quad
  ...
  { \Gamma \vdash_\tau \RuptEvent^{\Adm}_{\ET}(\tau\to\tau'; a) \text{ type} }
\end{mathpar}

where $\RuptEvent := \NoCarry \times \RuptPayload$.
...
\begin{tcolorbox}[colback=gray!5, colframe=gray!50, title=\textbf{Naming Convention: $\ET_L(\tau)$}]
We adopt a subscript convention to track granularity explicitly:

\medskip
\begin{tabular}{lll}
\textbf{Symbol} & \textbf{Definition} & \textbf{Vertices are...} \\
\hline
$\ET_{\mathrm{basin}}(\tau)$ & $\Ex^\infty N(\Ucov_\tau)$ & Basins (Čech nerve) \\
$\ET_{\mathrm{tok}}(\tau)$ & $\Ex^\infty K_{\mathrm{Tok}}(R)$ & Tokens (Dowker complex) \\
$\ET_{\mathrm{bar}}(\tau)$ & $\Ex^\infty N_{\mathrm{bar}}(\tau)$ & Witnessed bars (Chapter~\ref{chap:bars}) \\
\end{tabular}

\medskip
When we write $\ET(\tau)$ without subscript in this chapter, we mean $\ET_{\mathrm{basin}}(\tau)$---the Kan replacement of the basin-level Čech nerve. Tokens \emph{inhabit} this space via their basin memberships $\mathrm{mem}(t) \subseteq J$.

\medskip
\textbf{Why two token-level options?} Chapter~\ref{chap:embedding-geometry} introduced both:
\begin{itemize}
  \item $\ET_{\mathrm{basin}}$: paths witness ``basins overlap'' (general coherence)
  \item $\ET_{\mathrm{tok}}$: edges witness ``tokens share a basin'' (strong coherence)
\end{itemize}
Both are valid instantiations of DHoTT. The choice encodes different theories of what counts as semantic continuity. The Generic Dynamic Schema is agnostic---it works for either, and for bars, and for any future granularity $L$.
\end{tcolorbox}

For the basin-level treatment in this chapter, $L = \mathrm{basin}$ and $E_{\mathrm{basin}}(\tau) = \ET(\tau) := \Ex^\infty N(\Ucov_\tau)$.
