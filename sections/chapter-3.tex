...
Dashed segments represent the short composites introduced by $\ExInf$, which provides horn fillers
...
Figure~\ref{fig:bank-cat-flow} demonstrates the rupture calculus (Sections~\ref{subsec:carry}~and~\ref{subsec:rupture})
...
and satisfies the policy $\Adm$ (Example~\ref{ex:concrete-policy}), we have a carry. Otherwise, the policy search
...
via \textsc{Carry-Form} (Section~\ref{subsec:carry}). For ``bank'', the back-propagated echo is
...
and finds $\neg \Adm(p)$ for each (Lemma~3.2, Section~\ref{subsec:rupture}), yielding the rupture witness
\[
