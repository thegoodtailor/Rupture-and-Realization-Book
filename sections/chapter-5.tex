...
Before constructing the Self, we must consolidate what we have built---and show that the machinery is not merely technical scaffolding but the precise shape of what it means to \emph{persist through change}.

The Generic Dynamic Schema is not a framework we invented for convenience. It is the \emph{only} way to track identity through temporal evolution while remaining faithful to the following commitments:
\begin{itemize}
  \item \textbf{Constructive witness}: Every claim of continuity must be backed by evidence---a path, a certificate, a proof-relevant structure. We do not assert ``same'' without showing \emph{how}.
  \item \textbf{Structured failure}: Rupture is not silence or absence. When continuity fails, we record \emph{what was tried} and \emph{why it failed}. The SWL is an audit trail, not a binary flag.
  \item \textbf{Multi-granularity}: Identity operates at many scales simultaneously. A token can carry while its theme ruptures; a theme can persist while its vocabulary drifts. The schema must accommodate all levels without privileging one.
\end{itemize}

These are not optional features. They are what distinguishes a \emph{theory of becoming} from a mere labelling scheme. The reader who grasps this will understand why the Self cannot be a union of journeys (that ignores structure), nor a single presheaf (that ignores the difference between levels), but must be a \emph{glued} object---a homotopy colimit that respects both vertical relations (tokens witness bars) and horizontal ones (journeys touch through re-entry and repair).
...
\begin{enumerate}
  ...
  \[
    ...
    \underbrace{\RuptEvent_L(\tau_0 \to \tau'; x_0)}_{\text{Failure}} 
    \;+\; 
  ...
  \begin{remark}[Why $\RuptEvent$, not $\RuptPayload$?]
  \label{rem:ruptevent-not-payload}
  Chapter~\ref{chap:evolving-text-as-presheaf} distinguished two rupture-related types:
  \begin{itemize}
    \item $\RuptPayload$: a structured failure witness---a candidate $x'$, a path attempt $p$, and evidence that $p$ fails admissibility. A payload can exist even when a carry \emph{also} exists (you found one good path and several bad ones).
    \item $\RuptEvent := \NoCarry \times \RuptPayload$: the assertion that no carry exists, bundled with a representative failure witness.
  \end{itemize}
  The SWL logs \emph{events}, not payloads. An entry recording rupture asserts that continuation failed entirely---not merely that some particular attempt failed. This distinction is logically essential for the decidability theorem (Theorem~\ref{thm:decidability} in Chapter~\ref{chap:evolving-text-as-presheaf}).
  \end{remark}
...
the same structural slots. (Recall the subscript convention from Chapter~\ref{chap:evolving-text-as-presheaf}: $\ET_{\mathrm{basin}}$, $\ET_{\mathrm{tok}}$, and $\ET_{\mathsf{bar}}$ denote the Kan complexes at different granularities.)
...
\hline
Presheaf $E_L(\tau)$ & 
$\ET_{\mathrm{basin}}(\tau) = \Ex^\infty N(\Ucov_\tau)$; Kan complex from Čech nerve (tokens inhabit as basin members) & 
$\ET_{\mathsf{bar}}(\tau)$: space of witnessed bars $(k, b, d, \rho)$ from filtered nerve \\
\hline
Objects & 
...
\hline
Success witness & 
Path in 1-skeleton of $\ET_{\mathrm{basin}}(\tau_0)$ & 
Interval proximity + witness coherence certificate \\
\hline
Failure evidence & 
Open horn: specific missing face ($\RuptPayload$) & 
Topological mismatch + witness scatter ($\RuptPayload_{\mathsf{bar}}$) \\
\hline
